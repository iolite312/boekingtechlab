<nav class="AdminNavigation">
    <a href="/admin/index" class="item <?php if($page=='admindashboard'){echo 'active';}?>">
        <i class=""></i><span>Dashboard </span>
    </a>
    <a href="/admin/pages/booking" class="item <?php if($page=='adminbooking'){echo 'active';}?>">
        <i class=""></i><span>Products</span>
    </a>
    <a href="/admin/pages/accounts" class="item <?php if($page=='adminaccounts'){echo 'active';}?>">
        <i class=""></i><span>News</span>
    </a>
    <a href="/admin/pages/materials" class="item <?php if($page=='adminmaterials'){echo 'active';}?>">
        <i class=""></i><span>Message</span>
    </a>
</nav>